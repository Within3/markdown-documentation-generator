...
function customRenderers(renderer, options) {
    ...
    renderer.code = function (text, lang) {
        //check for example identifier
        if (lang === options.exampleIdentifier || options.exampleIdentifier.indexOf(lang) > -1){
            if (options.examplePreprocessor) {
                try {
                    text = options.examplePreprocessor(text);
                }
                catch(err) {
                    text = err.message;
                    log.generic(_sg.error('Error in examplePreprocessor: ') + err.message, 2);
                }
            }
