...
function register(renderer, options) {
    ...
    renderer.heading = function (string, number) {
        if (number === 1) {
            ...
            return '<h' + number + ' class="sg-heading sg-heading-'+ number +
                '" id="heading-' + sanitizedId + '">' + string + '</h' + number + '>\n';
        }
    };
    ...
    renderer.code = function (text, lang) {
        ...
        if (lang === options.exampleIdentifier){
            ...
            return '\n<div class="sg-markup sg-codeblock">\n' +
                '<pre class="sg-markup_wrap"><code class="hljs sg-code">' +
                processedText +
                '\n</code></pre></div>\n';
        }
    };
        ...
        for (var check in codeChecks) {
            if ({}.hasOwnProperty.call(codeChecks, check)){
                ...
                if (test !== null){
                    ...
                    return '<code id="'+check+'-'+safeString+'" class="sg-global-'+ check +
                        classString+'" data-code-id="'+newString+'">'+newString+'</code>';
                }
            }
        }
...
module.exports = {
    register: function(renderer, options){return register(renderer, options);}
};
