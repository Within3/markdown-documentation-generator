...
sanitize.removeDiacritics = function(str) {
    return str.replace(/[^\u0000-\u007E]/g, function(a){
        return diacriticsMap[a] || a;
    });
};

sanitize.makeUrlSafe = function(str) {
    var ret = str || '';
    ret = ret.toLowerCase()
        .replace(/ /g, '_')
        .replace(/(\")/g,'')
        ...
        .replace(')', '')
        .replace('=', '');
    ret = sanitize.removeDiacritics(ret);
    return ret;
};

sanitize.makeSafeForCSS = function(name) {
    name = name.replace(/[\@\(\)]/gi, '');

    return name.replace(/<\/?[a-z][a-z0-9]*[^<>]*>|<!--.*?-->/gi, function(s) {
        return '_';
    });
};

//Unnest Arrays
sanitize.unnest = function(array, levels) {
    levels = levels || 1;
    var empty = [];
    ...
    if (levels && levels - 1) {
        return sanitize.unnest(array, levels - 1);
    }
 ...
 */
sanitize.objectPush = function (obj, elem) {
    [].push.call(obj, elem);
};
 ...
 */
sanitize.cheerioWrapAll = function($){
    _.extend($.prototype, {
...
sanitize.path = function(from, to){
    return path.relative(from, path.join(__dirname, '../', to));
};
